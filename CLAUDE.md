...
- SCStream sample handler (via `SCStreamProxy`, a thin NSObject adapter) dispatches to `audioQueue.async` with `assumeIsolated` for stereo→mono downmix and writing via RecordingPipeline
