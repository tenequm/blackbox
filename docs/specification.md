...
- **Under `.defaultIsolation(MainActor.self)`, `nonisolated` does not get async work off the main thread.** Marking a class `nonisolated` is not enough, and neither is calling it through a protocol: the async method still runs on the caller's executor, main actor included. Only `@concurrent` moves it, and putting `@concurrent` on the *protocol requirement* covers every conformance - class and actor witnesses alike - which is why `TranscriptionServicing` annotates all five of its requirements. Verified against this target's build flags; without it, transcript JSON decoding and poll responses parse on the main thread. Note the project does **not** enable the `NonisolatedNonsendingByDefault` upcoming feature, so guidance written against that flag does not describe this target.
...
**Consent is re-checked on resume, not just at trigger.** At quit the monitor reports saved recordings *before* clearing `isRecording`, so `pump()` always defers behind the recording gate and the process exits having written only a sidecar. That makes resume the normal path for quit-time recordings, not an edge case - so resume re-applies the gate: an automatic job that has not uploaded anything yet is dropped if `autoTranscribe` is now off. A job that *has* uploaded is finished either way, because the audio is already on Soniox and completing it beats stranding it there. Manual jobs resume regardless; the user asked for them explicitly.

**Which audio file:** `audio-processed.m4a` when echo cancellation has been run, otherwise `audio.m4a`, resolved at job run time rather than enqueue time. One rule, shared with playback and the storage stats via `RecordingStore.audioURL(in:)`, and it lets a resumed job pick up an AEC pass that finished in between.

**Failure policy:** Offline is a wait, not a failure - it retries on a fixed interval without consuming an attempt, bounded separately from the retry backstop. Transient failures (5xx, 429, `pollTimeout`, dropped connections) retry three times with 30s/2m/8m backoff. Everything else is terminal: the sidecar is kept with `lastError` set so the recording row shows the failure and the detail view offers a retry, and launch resume deliberately surfaces those jobs instead of re-running them. Running out of passes is *not* terminal - the sidecar is left resumable, because the last thing seen was retryable. A transcript that cannot be written to disk is reported as a failure rather than retried, because the paid-for result is already in hand.

**Nothing is left on Soniox.** Success, terminal failure, and cancellation all call `deleteRemote`. A terminal failure additionally clears `fileId`/`transcriptionId` from the sidecar so a retry starts clean, and cancelling a *queued* job loads its sidecar first - dropping it without that would strand audio on Soniox with nothing left on disk that could name it. The one residual: if a request fails after the multipart body was fully transmitted but before the response arrives, Soniox may hold a file whose id was never learned. Soniox has no idempotency key, so this is bounded (at most the 3 retry attempts) rather than eliminated; every transcription carries `client_reference_id` set to the recording directory name, so a stray job is at least identifiable in the Soniox console.

**Error mapping follows the published contract, not inference.** Soniox answers every non-2xx with `{status_code, error_type, message, validation_errors, request_id}`; `request_id` is logged because it is what their support asks for. Status handling: **409** `transcription_invalid_state` is `.notReady` and retryable - polling and fetching are separate calls, so a job that completes between them answers 409 for a moment, and treating that as terminal would kill a job that was about to succeed. **402** is `.balanceExhausted` and terminal, because retrying a dead balance only wastes time. **429** and **5xx** are `.serverError` and retryable. Everything else is terminal. The transcription status enum is `queued | processing | completed | error` - note there is no `transcribing`, which an earlier revision branched on and therefore never matched.

**The model id is configuration, not a constant** (`sonioxModel`, default `stt-async-v5`), so a model retirement does not need an app release. `stt-async-v4` is currently an alias for v5 and retires 2026-06-30.
...
**Quit:** Nothing on this path joins `applicationShouldTerminate`'s 8s budget. An in-flight job dies with the process and is picked up by `resumePendingJobs()` on the next launch, which also sweeps the mix and upload scratch files a killed job leaves in the temp directory - each is the full size of a recording.
